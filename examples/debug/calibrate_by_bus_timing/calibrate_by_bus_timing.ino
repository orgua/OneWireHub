...
auto ds18b20 =
        DS18B20(DS18B20::family_code, 0x00, 0x02, 0x0B, 0x08, 0x01, 0x0D); // Digital Thermometer
...
void loop()
{
    ...
    if (false)
    {
        ...
        while (loops_left)
        {
            ...
            uint32_t retries =
                    loops_1ms; // volatile needs ~2.5ms for the 1ms-loop .... but in a separate function it works as expected
            while ((!DIRECT_READ(debug_baseReg, debug_bitMask)) && (--retries))
                ; // try to wait 1ms while LOW

            DIRECT_WRITE_HIGH(debug_baseReg, debug_bitMask); // switch to HIGH

            retries += loops_1ms;
            while ((DIRECT_READ(debug_baseReg, debug_bitMask)) && (--retries))
                ; // try to wait 1ms while HIGH
            loops_left = retries;
