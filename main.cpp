...
void test_eq(const T1 value_A, const T2 value_B, const string message)
{
    if (value_A != value_B)
    {
        cout << "- FAIL (" << to_string(value_A) << " != " << to_string(value_B) << ") " << message
             << endl;
        tests_failed++;
    }
    tests_absolved++;
...
int main()
{
    ...
    auto ds2408 =
            DS2408(0x29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00); //      - 8-Channel Addressable Switch
    auto ds2413 = DS2413(0x3A, 0x0D, 0x02, 0x04, 0x01, 0x03,
                         0x00); // Work - Dual channel addressable switch

    auto ds2401c =
            DS2401(0x01, 0x00, 0x0D, 0x24, 0x01, 0x00, 0x0C); // additional device for testing

    auto ds2423 =
            DS2423(0x1D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00); //      - 4kb 1-Wire RAM with Counter
    ...
    auto ds2450 =
            DS2450(DS2450::family_code, 0x00, 0x00, 0x50, 0x24, 0xDA, 0x00); //      - 4 channel A/D
...

    {
        ...
        const std::array<float, 10>   temp_degC{125.0f, 85.0f, 25.0625f, 10.125f,   0.5f,
                                              0.0f,   -0.5f, -10.125f, -25.0625f, -55.0f};
        const std::array<int16_t, 10> temp_raw{0x07D0,     0x0550,    0x0191,     0x00A2,
                                               0x0008,     0x0000,    0xFFF8_i16, 0xFF5E_i16,
                                               0xFE6F_i16, 0xFC90_i16};

        for (size_t index{0}; index < temp_degC.size(); ++index)
        {
            ds18B20.setTemperature(temp_degC[index]);
            test_eq(ds18B20.getTemperatureRaw(), temp_raw[index],
                    "DS18B22 datasheet test" + to_string(index));
        }
    }

    {
        ...
        const std::array<int16_t, 7> temp_raw{0x00AA,     0x0032,     0x0001,    0x0000,
                                              0xFFFF_i16, 0xFFCE_i16, 0xFF92_i16};

        for (size_t index{0}; index < temp_degC.size(); ++index)
        {
            ds18S20.setTemperature(temp_degC[index]);
            test_eq(ds18S20.getTemperatureRaw(), temp_raw[index],
                    "DS18S22 datasheet test" + to_string(index));
        }
    }
...

    {
        ...
        for (const auto value: values_pin)
        {
            ...
            test_eq(ds2413.getPinState(value), false,
                    "DS2413 change in state of pin " + to_string(value) + " because of latch");
            ds2413.setPinState(value, true);
            test_eq(ds2413.getPinState(value), false,
                    "DS2413 re-enable state of pin " + to_string(value) +
                            " fails because pin still latched");

            ds2413.setPinLatch(value, false);
            test_eq(ds2413.getPinState(value), false,
                    "DS2413 state of pin " + to_string(value) + " after disabling latch");

            ds2413.setPinState(value, true);
            test_eq(ds2413.getPinState(value), true,
                    "DS2413 re-enable state of pin " + to_string(value) +
                            " after disabling latch works");
        }
    }

    {
        ...
        constexpr uint8_t mem_dummy[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        ...
        test_eq(ds2423.getCounter(0), 1_u32,
                "DS2423 write counter 0 - increment by writing register");
        ...
        ds2423.writeMemory(mem_dummy, sizeof(mem_dummy),
                           12 * 32 + 17); // second half of page 12 and 1 byte of page 13
        test_eq(ds2423.getCounter(0), 2_u32,
                "DS2423 write counter 0 - increment by writing register");
        test_eq(ds2423.getCounter(1), 2001_u32,
                "DS2423 write counter 1 - increment by writing register");
        ...
        {
            test_eq(mem_read[index], memory[index],
                    "DS2423 mem re-read at position A " + to_string(index));
        }
        ...
        {
            test_eq(mem_read[index], mem_dummy[index - 1],
                    "DS2423 mem re-read at position B " + to_string(index));
        }
    }

    {
        ...
        {
            test_eq(mem_read[index], memory[index],
                    "DS2431 mem re-read at position A " + to_string(index));
        }
        ...
        {
            test_eq(mem_read[index], mem_dummy[index - 1],
                    "DS2431 mem re-read at position B " + to_string(index));
        }
...

    {
        ...
        {
            test_eq(mem_read[index], memory[index],
                    "DS2433 mem re-read at position A " + to_string(index));
        }
        ...
        {
            test_eq(mem_read[index], mem_dummy[index - 1],
                    "DS2433 mem re-read at position B " + to_string(index));
        }
    }

    {
        ...
        {
            test_eq(mem_read[index], memory[index],
                    "DS2438 mem re-read at position A " + to_string(index));
        }

        ds2438.readMemory(mem_read, sizeof(mem_dummy),
                          4 * 8 - 1); // begin one byte earlier than page 1
        for (size_t index = 1; index < sizeof(mem_dummy); ++index)
        {
            test_eq(mem_read[index], mem_dummy[index - 1],
                    "DS2438 mem re-read at position B " + to_string(index));
        }
    }
            ...
            for (size_t index = 0; index < (sizeof(test_data) / 2); ++index)
            {
                ...
                for (uint8_t poti_read = 0; poti_read < 4; ++poti_read)
                {
                    const uint16_t result =
                            (poti_read == poti_write) ? test_data[index] : uint16_t(77);
                    test_eq(ds2450.getPotentiometer(poti_read), result,
                            "DS2450 test for poti " + to_string(poti_write) + " with poti " +
                                    to_string(poti_read));
                }
            }
        }
...

    {
        ...
        {
            ds2502.readMemory(mem_read, sizeof(mem_read),
                              page * 32_u8); // begin one byte earlier than page 1
        ...
        ds2502.readMemory(mem_read, sizeof(mem_read),
                          1_u8 * 32_u8); // begin one byte earlier than page 1
        for (size_t index = 0; index < sizeof(mem_read); ++index)
        {
        ...
        ds2502.readMemory(mem_read, sizeof(mem_read),
                          3_u8 * 32_u8); // begin one byte earlier than page 1
        for (size_t index = 0; index < sizeof(mem_read); ++index)
        {
...

    {
        ...
        {
            test_eq(ds2506.getRedirectionProtection(page), false,
                    "DS2506 get protection for redirection (not modified yet) for page " +
                            to_string(page));
        }
        ...
        for (uint8_t page = 0; page < 8; ++page)
        {
            ...
            test_eq(ds2506.getRedirectionProtection(page), result,
                    "DS2506 get protection for redirection (modified) for page " + to_string(page));
        }


        for (uint8_t page = 0; page < 8; ++page)
        {
            test_eq(ds2506.getPageRedirection(page), 0_u8,
                    "DS2506 get page redirection (not modified yet) for page " + to_string(page));
        }
        ...
        for (uint8_t page = 0; page < 8; ++page)
        {
            ...
            test_eq(ds2506.getPageRedirection(page), result,
                    "DS2506 get page redirection (modified) for page " + to_string(page));
        }

        for (uint8_t page = 0; page < 8; ++page)
        {
            test_eq(ds2506.getPageUsed(page), false,
                    "DS2506 get page usage (not modified yet) for page " + to_string(page));
        }
        ...
        for (uint8_t page = 0; page < 8; ++page)
        {
            ...
            test_eq(ds2506.getPageUsed(page), result,
                    "DS2506 get page usage (modified) for page " + to_string(page));
        }

        ds2506.readMemory(mem_read, sizeof(mem_read), 4_u8 * 32_u8);
        for (size_t index = 0; index < sizeof(mem_read); ++index)
        {
            test_eq(mem_read[index], mem_dummy[index],
                    "DS2506 mem re-read page 4 - previously written");
        }
        ...
        {
            test_eq(mem_read[index], mem_dummy[index],
                    "DS2506 mem re-read page 1 - previously written");
        }
    }
        ...
        for (uint8_t poti_write = 0; poti_write < 4; ++poti_write)
        {
            for (size_t index = 0; index < sizeof(test_data); ++index)
            {
                ...
                for (uint8_t poti_read = 0; poti_read < 4; ++poti_read)
                {
                    const uint16_t result =
                            (poti_read == poti_write) ? test_data[index] : uint16_t(77);
                    test_eq(ds2890A.getPotentiometer(poti_read), result,
                            "DS2890 test for poti " + to_string(poti_write) + " with poti " +
                                    to_string(poti_read));
                }
            }
        }
...

    {
        ...
        // add default-data
        constexpr uint8_t
                mem1[24] = {0x14, 0x10, 0x90, 0xd0, 0x03, 0x32, 0x4b, 0x3c, 0xff,
                            0x04, 0x64, 0x04, 0x9e, 0x9a, 0x3a, 0xf0, 0x20, 0x20,
                            0x04, 0xee, 0x63, 0xB8, 0x3E, 0x63}; // last 4 Byte seem to be Serial
...

    {
...

        {
            ...
            {
                write_bits_A[index++] =
                        ((bitMask & data_test) != 0); // TODO: shifting value could be faster
            }
        }

        {
            ...
            {
                write_bits_B[index++] =
                        ((value & 0x01) != 0); // TODO: shifting value could be faster
                value >>= 1;
            }
        }
        ...
        {
            test_eq(write_bits_B[index], write_bits_A[index],
                    "algo without bitmask on position " + to_string(index));
        }
    }

    {
...

        {
            ...
            for (uint8_t index = 0; index < data_size; index++) { data_array[index] = index; }
        }
...

        {
            ...
            while (_size-- > 0) { *data_array++ = _index++; }
        }

        for (size_t index = 0; index < data_size; ++index)
        {
            test_eq(data_array_B[index], data_array_A[index],
                    "algo without index B on position " + to_string(index));
            test_eq(data_array_C[index], data_array_A[index],
                    "algo without index C on position " + to_string(index));
        }
    }
    ...
    cout << "Program did run " << tests_absolved << " tests, " << tests_failed << " of them failed."
         << endl;
