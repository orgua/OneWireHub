...
public:
    ...
    DS2413(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
           uint8_t ID7);
    ...
    bool getPinState(const uint8_t a_or_b) const { return pin_state[a_or_b & 1]; }

    void setPinLatch(const uint8_t a_or_b,
                     const bool    value) // latching a pin will pull it down (state=zero)
    {
        pin_latch[a_or_b & 1] = value;
        if (value) setPinState(a_or_b, false);
    }

    bool getPinLatch(const uint8_t a_or_b) const { return pin_latch[a_or_b & 1]; }
};
