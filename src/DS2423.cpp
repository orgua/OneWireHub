...
DS2423::DS2423(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
...
{
    constexpr uint32_t DUMMY_32b_ZERO{
            0x00000000}; // should be std::numeric_limits<uint32_t>::lowest()
    ...
    switch (cmd)
    {
        case 0x0F: // Write Scratchpad
            ...
            for (; reg_ES < PAGE_SIZE; ++reg_ES)
            {
                ...
                {
                    if (hub->getError() == Error::AWAIT_TIMESLOT_TIMEOUT_HIGH)
                        reg_ES |= REG_ES_PF_MASK;
                    break;
                }
            }
            reg_ES--;
        ...
        case 0x5A: // copy scratchpad
            ...
            while (!hub->send(&ALTERNATING_10))
                ; // send 1s when alternating 1 & 0 after copy is complete
            break;
        ...
        case 0xA5: // Read Memory + Counter
...

            {
                ...
                for (; page < PAGE_COUNT; ++page)
                {
                    ...
                    {
                        if (hub->send(reinterpret_cast<uint8_t *>(
                                              &memcounter[page - COUNTER_PAGE_START]),
                                      4, crc))
                            return;
                    }
                    else
                    {
                        if (hub->send(reinterpret_cast<const uint8_t *>(&DUMMY_32b_ONES), 4, crc))
                            return;
                    }
                    if (hub->send(reinterpret_cast<const uint8_t *>(&DUMMY_32b_ZERO), 4, crc))
                        return;
                         ...
                         const uint16_t position)
{
    ...
    for (uint8_t page = page_start; page <= page_end;
         ++page) // page 12 & 13 have write-counters, page 14&15 have hw-counters
    {
        if ((page == COUNTER_PAGE_START) || (page == COUNTER_PAGE_START + 1))
            memcounter[page - COUNTER_PAGE_START]++;
    }
