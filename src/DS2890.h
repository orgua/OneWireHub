...
public:
    ...
    DS2890(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
           uint8_t ID7);
    ...
    uint8_t getPotentiometer(uint8_t channel) const { return register_poti[channel & POTI_MASK]; }

    uint8_t getRegCtrl(void) const { return register_ctrl; }

    uint8_t getRegFeat(void) const { return register_feat; }
};
