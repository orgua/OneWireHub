...
DS2408::DS2408(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
...
void DS2408::setPinState(const uint8_t pinNumber, const bool value)
{
    ...
    memory[REG_PIO_ACTIVITY] |=
            pio_state ^
            memory[REG_PIO_LOGIC]; // TODO: just good guess here, has anyone the energy to figure out each register?
...
uint8_t DS2408::getPinState(void) const { return memory[REG_PIO_LOGIC]; }
...
uint8_t DS2408::getPinActivity(void) const { return memory[REG_PIO_ACTIVITY]; }
