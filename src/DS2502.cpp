...
DS2502::DS2502(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
...
bool DS2502::setPageRedirection(const uint8_t page_source, const uint8_t page_destin)
{
    ...
    status[page_source + STATUS_PG_REDIR] =
...
uint8_t DS2502::getPageRedirection(const uint8_t page) const
{
    ...
    return ~(
            status[page +
                   STATUS_PG_REDIR]); // TODO: maybe invert this in ReadStatus and safe some Operations? Redirection is critical and often done
}
