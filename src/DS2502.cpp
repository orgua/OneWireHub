...
void DS2502::duty(OneWireHub *hub)
{
    ...
    uint8_t  reg_RA = 0, cmd, data, crc = 0; // redirected address, command, data, crc
    ...
    switch (cmd)
    {
        ...
        case 0xC3:      // READ DATA (like 0xF0, but repeatedly till the end of page with following CRC)
            ...
            {
                crc = 0; // reInit CRC and send data
                reg_RA = uint8_t((reg_TA & ~page_mask) + (1 << 5));
...
{
    const uint16_t _length = (position + length >= sizeof_memory) ? (sizeof_memory - position) : length;
