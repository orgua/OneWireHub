...
using io_reg_t = uint8_t; // define special datatype for register-access
constexpr uint8_t VALUE_IPL{
        0}; // instructions per loop, uncalibrated so far - see ./examples/debug/calibrate_by_bus_timing for an explanation
...
constexpr uint8_t VALUE_IPL{
        22}; // instructions per loop, uncalibrated so far - see ./examples/debug/calibrate_by_bus_timing for an explanation
...
constexpr uint8_t VALUE_IPL{
        39}; // instructions per loop, for 40 and 80 MHz (see esp8266 difference)
...
{
    io_reg_t ret;
    if (SS_GPIO == GPIO_TYPE(pin)) { ret = READ_ARC_REG(((io_reg_t) base + EXT_PORT_OFFSET_SS)); }
    else { ret = MMIO_REG_VAL_FROM_BASE((io_reg_t) base, EXT_PORT_OFFSET_SOC); }
    return ((ret >> GPIO_ID(pin)) & 0x01);
}

static inline __attribute__((always_inline)) void directModeInput(volatile io_reg_t *base,
                                                                  io_reg_t pin)
{
    ...
    else { MMIO_REG_VAL_FROM_BASE((io_reg_t) base, DIR_OFFSET_SOC) &= ~(0x01 << GPIO_ID(pin)); }
}

static inline __attribute__((always_inline)) void directModeOutput(volatile io_reg_t *base,
                                                                   io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
...
static inline __attribute__((always_inline)) void directWriteLow(volatile io_reg_t *base,
                                                                 io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
    ...
    else { MMIO_REG_VAL(base) &= ~(0x01 << GPIO_ID(pin)); }
}

static inline __attribute__((always_inline)) void directWriteHigh(volatile io_reg_t *base,
                                                                  io_reg_t pin)
{
    if (SS_GPIO == GPIO_TYPE(pin))
    {
...
using io_reg_t = uint32_t; // define special datatype for register-access
constexpr uint8_t VALUE_IPL{
        10}; // instructions per loop, uncalibrated so far - see ./examples/debug/calibrate_by_bus_timing for an explanation
...
void memset(T1 *const address, const T1 initValue, const T2 bytes)
{
    ...
    for (T2 counter = 0; counter < iterations; ++counter) { address[counter] = (initValue); }
}
...
template<typename T1, typename T2>
bool memcmp(const T1 *const source_A, const T1 *const source_B,
            const T2 bytes) // return true if string is different
{
    const T2 iterations = bytes / sizeof(T1);
    for (T2 counter = 0; counter < iterations; ++counter)
    {
