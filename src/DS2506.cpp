...
DS2506::DS2506(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
    : OneWireItem(ID1, ID2, ID3, ID4, ID5, ID6, ID7)
{
    static_assert(sizeof(memory) <= 0xFFFF,
        ...
        case 0xA5: // EXTENDED READ MEMORY (with redirection-information)
            ...
            while (reg_TA <= sizeof_memory)
            {
                ...
                hub->send(
                        ...
                        2); // send crc of (cmd,TA,destin_page) at first, then only crc of (destin_page)
                crc                      = 0;
                const uint16_t destin_TA = translateRedirection(reg_TA);
                const uint8_t  length    = PAGE_SIZE - uint8_t(reg_TA & PAGE_MASK);
...
uint16_t DS2506::translateRedirection(const uint16_t source_address)
        const // TODO: extended read mem description implies that redirection is recursive
{
...
uint8_t DS2506::readStatus(const uint16_t address) const
{
    ...
    {
        if (SA < STATUS_SEGMENT)
            return_value = status[SA + 0 * STATUS_SEGMENT]; // emulate protection
    }
    else if (address < STATUS_PG_WRITN_BEG) // is WP_REDIR
    {
        SA -= STATUS_WP_REDIR_BEG;
        if (SA < STATUS_SEGMENT)
            return_value = status[SA + 1 * STATUS_SEGMENT]; // emulate protection
    }
...
bool DS2506::setPageRedirection(const uint8_t page_source, const uint8_t page_destin)
{
    ...
    status[3 * STATUS_SEGMENT + page_source] =
            (page_destin == page_source)
                    ? uint8_t(0xFF)
                    : ~page_destin; // datasheet dictates this, so no page can be redirected to page 0
    return true;
}
...
uint8_t DS2506::getPageRedirection(const uint8_t page) const
{
    ...
    return (~status[3 * STATUS_SEGMENT +
                    page]); // TODO: maybe invert this in ReadStatus and safe some Operations? Redirection is critical and often done
}
