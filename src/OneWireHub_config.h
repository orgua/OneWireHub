...
constexpr bool USE_GPIO_DEBUG{
        false}; // is a better alternative to serial debug (see readme.md for info) SHOULD NOT be enabled with < 20 MHz uC and Overdrive enabled
constexpr uint8_t  GPIO_DEBUG_PIN{7}; // digital pin
constexpr uint32_t REPETITIONS{
        5000}; // for measuring the loop-delay --> 10000L takes ~110ms on atmega328p@16Mhz
...
static_assert(!(USE_GPIO_DEBUG && (microsecondsToClockCycles(1) < 20) && (OVERDRIVE_ENABLE != 0)),
...
constexpr timeOW_t ONEWIRE_TIME_MSG_HIGH_TIMEOUT = {
        15000_us}; // there can be these inactive / high timeperiods after reset / presence, this value defines the timeout for these
