...
DS2438::DS2438(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
...
void DS2438::clearMemory(void)
{
    ...
    for (uint8_t page = 0; page < PAGE_COUNT; ++page) { calcCRC(page); }
}
...
int8_t DS2438::getTemperature() const { return memory[2]; }
...
void DS2438::setVDDVoltage(uint16_t voltage_10mV) { convertVoltage(vddVoltage, voltage_10mV); }

void DS2438::setVADVoltage(uint16_t voltage_10mV) { convertVoltage(vadVoltage, voltage_10mV); }

uint16_t DS2438::getVoltage(void) const { return ((memory[4] << 8) | memory[3]); }
...
int16_t DS2438::getCurrent(void) const { return ((memory[6] << 8) | memory[5]); }
