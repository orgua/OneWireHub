...
private:
    ...
    static constexpr uint8_t STATUS_PG_WRITN_BEG{
            0x40}; // 32  bytes -> Page used status (written ones)
    static constexpr uint8_t  STATUS_UNDEF_B1_BEG{0x60};
    static constexpr uint16_t STATUS_PG_REDIR_BEG{
    ...
    uint16_t translateRedirection(uint16_t source_address)
            const; // react to redirection in status and not available memory

public:
    static constexpr uint8_t family_code{0x0F}; // the ds2506

    DS2506(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
           uint8_t ID7);
