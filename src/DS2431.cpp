...
DS2431::DS2431(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
    ...
    switch (cmd)
    {
        case 0x0F: // WRITE SCRATCHPAD COMMAND
            ...
            if (reg_TA < (4 * PAGE_SIZE)) // check if page is protected or in eprom-mode
            {
                ...
                if (getPageProtection(reinterpret_cast<uint8_t *>(
                            &reg_TA)[0])) // protected: load memory-segment to scratchpad
                {
                    for (uint8_t i = 0; i < SCRATCHPAD_SIZE; ++i)
                        scratchpad[i] = memory[position + i];
                }
                else if (getPageEpromMode(reinterpret_cast<uint8_t *>(
                                 &reg_TA)[0])) // eprom: logical AND of memory and data
                {
                    for (uint8_t i = page_offset; i <= reg_ES; ++i)
                        scratchpad[i] &= memory[position + i];
                }
            }
            break;
        ...
        case 0xAA: // READ SCRATCHPAD COMMAND
            ...
            { // send Scratchpad content
                ...
                const uint8_t length =
                        (reg_ES & SCRATCHPAD_MASK) + uint8_t(1) - start; // difference to ds2433
...
void DS2431::clearMemory(void) { memset(memory, static_cast<uint8_t>(0x00), sizeof(memory)); }
