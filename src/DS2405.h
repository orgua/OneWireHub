...
public:
    ...
    DS2405(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
           uint8_t ID7);

    void duty(OneWireHub *hub) final;

    void setPinState(const bool value) { pin_state = value; }

    bool getPinState(void) const { return pin_state; }
};
